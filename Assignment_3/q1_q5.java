...
public class q1_q5 {
    public static void main(String[] args) {
        ...
        try {
            ...
            String ip = "a1b2c3d4e5";
            if (ip == null || ip.isEmpty()) {
                throw new NullPointerException("Input string is null or empty");
            }
            boolean found = false;
            for (int i = 0; i < ip.length() - 2; i++) {
                char prevChar = ip.charAt(i);
                char currentChar = ip.charAt(i + 1);
                char nextChar = ip.charAt(i + 2);
        ...
        } catch (CustomNullPointerException error) {
        ...
        try {
            ...
            System.out.println("FileNotFoundException occurred: " + error.getMessage());
        } catch (FileReadPermissionException error) {
            System.out.println("Task 4: Custom FileNotFoundException and FileReadPermissionException classes");
            System.out.println("FileReadPermissionException occurred: " + error.getMessage());
        }
    }
